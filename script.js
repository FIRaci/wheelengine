document.addEventListener('DOMContentLoaded', () => {
    ...
    // Phase 18: Advanced Remove Items Elements
    ...
    function loadState(stateObject, fromFile = false) {
        ...
        Object.values(variableTemplate).forEach(v => {
            if (typeof v.displayInStats === 'undefined') v.displayInStats = true;
        });
        ...
        conditionalRules.forEach(rule => {
             if (rule.conditions) {
                ...
                delete rule.conditions; delete rule.actions;
            }
        });
        
        const firstCollectionId = Object.keys(collectionTemplate)[0];
        Object.values(itemDatabase).forEach(item => {
            if (!item.collectionId && firstCollectionId) item.collectionId = firstCollectionId;
        ...
        Object.values(entities).forEach(entity => {
            if (!entity.collections) entity.collections = {};
             Object.keys(collectionTemplate).forEach(collectionId => {
                if(!entity.collections[collectionId]) entity.collections[collectionId] = [];
             });
        });
        ...
        // Data migration for Phase 18: removeItems action
        const migrateActions = (actions) => {
            if (!actions) return [];
            return actions.map(action => {
                if (action.type === 'removeRandomItem') {
                     return { type: 'removeItems', targetCollection: action.targetCollection || action.target, mode: 'random', amountType: 'specific', amountValue1: '1' };
                }
                if (action.type === 'clearCollection') {
                    return { type: 'removeItems', targetCollection: action.targetCollection || action.target, amountType: 'all' };
                }
                return action;
            });
        };
        Object.values(wheelsData).forEach(wheel => {
            if(wheel.segments) wheel.segments.forEach(segment => segment.actions = migrateActions(segment.actions));
        });
        Object.values(macros).forEach(macro => macro.actions = migrateActions(macro.actions));
        conditionalRules.forEach(rule => {
            if(rule.blocks) rule.blocks.forEach(block => block.actions = migrateActions(block.actions));
        });
    ...
    function setPresentationMode(enabled) {
        ...
        appContainer.classList.toggle('presentation-mode', isPresentationMode);
        const toggleModeText = document.getElementById('toggle-mode-text');
        const icon = toggleModeBtn.querySelector('svg');

        if (isPresentationMode) {
            toggleModeText.textContent = "Sửa";
            icon.innerHTML = `<path d="M0 2a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v12a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2V2zm15 2h-4v3h4V4zm-5 0H6v3h4V4zM6 9v3h4V9H6zm5 0v3h4V9h-4zM5 4H1v3h4V4zm-4 5v3h4V9H1z"/>`;
            navLinks[0].click();
        } else {
            toggleModeText.textContent = "Chơi";
            icon.innerHTML = `<path d="M11.596 8.697l-6.363 3.692c-.54.313-1.233-.066-1.233-.697V4.308c0-.63.692-1.01 1.233-.696l6.363 3.692a.802.802 0 0 1 0 1.393z"/>`;
        }
    }
    ...
    function renderLogicMap() {
        ...
        Object.keys(wheelsData).forEach(wheelName => {
            ...
            if (wheel.settings?.defaultLink && wheel.settings.defaultLink !== 'None') {
                edges.push({ from: wheelName, to: wheel.settings.defaultLink, label: 'Liên kết mặc định', arrows: 'to', dashes: true });
            }
        });
        ...
        const options = {
            edges: { color: "#999", font: { color: '#efefef', size: 12, strokeWidth: 0 } },
            nodes: { color: { background: '#404040', border: '#ffde59', highlight: { background: '#ffde59', border: '#ffc107' } }, font: { color: '#f0f0f0', size: 14 } },
            physics: true, // Default to physics layout
        };

        // FIX: Only use hierarchical layout if there are edges to direct the graph.
        if (edges.length > 0) {
    ...
    function evaluateFormula(formula, tempVariables = {}, contextEntityId = activeEntityId) {
        ...
        catch (e) { console.warn(`Error evaluating formula "${formula}":`, e); return formula; }
    }
            ...
            if (item && item.effects) {
                item.effects.forEach(effect => {
                    if (entity.variables[effect.target]) {
                        ...
                        if (effect.operator === '+=') entity.variables[effect.target].bonus += value;
                        else if (effect.operator === '-=') entity.variables[effect.target].bonus -= value;
                    }
                });
            }
        });
    ...
    function evaluateAllRules(depth = 0) {
        ...
        computedVariables.forEach(compVar => {
            ...
            if (!targetVarName || !activeEntity.variables[targetVarName]) return;
            ...
            if (newBonus !== oldValue) { targetVar.bonus = newBonus; changedInLoop = true; }
        });
        ...
        conditionalRules.forEach(rule => {
            for (const block of rule.blocks) {
                ...
                if (conditionsMet) {
                    ...
                    if (executeActions(block.actions)) changedInLoop = true;
                    break; 
                }
            }
        });
        ...
        (actions || []).forEach(action => {
            switch (action.type) {
                case 'calculateAssign': {
                    if(!action.targetTempVar || !action.calcFunction || !action.sourceCollection) break;
                    let result = 0;
                    const collection = activeEntity.collections[action.sourceCollection] || [];
                    if(action.calcFunction === 'COUNT') result = collection.filter(item => item).length;
                ...
                case 'setVariable': {
                    ...
                    switch (operator) {
                        ...
                        case 'invert': {
                            const min = evaluateFormula(action.min, tempVariables), max = evaluateFormula(action.max, tempVariables);
                            finalValue = (typeof min === 'number' && typeof max === 'number') ? (min + max) - (oldValue || 0) : oldValue;
                            break;
                        }
                    ...
                    if (finalValue !== oldValue) {
                        addLogMessage(`-> <span class="log-highlight">${varName}.${targetProperty}</span>: ${oldValue} -> ${finalValue}`);
                        activeVars[varName][targetProperty] = finalValue;
                        dataChanged = true;
                    }
                    break;
                }
                case 'setCollectionSlots': {
                    ...
                    if (!isNaN(newSize) && newSize >= 0) {
                        ...
                        const newSlots = Array(newSize).fill(null).map((_, i) => currentSlots[i] || null);
                ...
                case 'setItemInSlot': {
                    ...
                    if (slotIndex >= 0 && slotIndex < slots.length) {
                        const finalValue = String(valWithResult).toUpperCase() === '[NONE]' ? null : valWithResult;
                ...
                case 'removeItems': {
                    ...
                    if (amountType === 'specific') amount = parseInt(evaluateFormula(action.amountValue1, tempVariables), 10);
                        ...
                        const min = parseInt(evaluateFormula(action.amountValue2, tempVariables), 10);
                        const max = parseInt(evaluateFormula(action.amountValue3, tempVariables), 10);
                        if (!isNaN(min) && !isNaN(max)) amount = Math.floor(Math.random() * (max - min + 1)) + min;
                    ...
                    if (indexesToRemove.length > 0) {
                         const removedItems = indexesToRemove.map(i => slots[i]);
                         indexesToRemove.forEach(i => slots[i] = null);
                         addLogMessage(`-> Đã xóa ${indexesToRemove.length} vật phẩm: <span class="log-highlight">${removedItems.join(', ')}</span> từ <span class="log-highlight">${collectionTemplate[targetCollection]?.name}</span>.`);
                         dataChanged = true;
                    }
                    break;
                }
    ...
    function createMacroActionRow(action = {}) { const row = document.createElement('div'); row.className = 'macro-action-row'; const collectionOptions = Object.values(collectionTemplate).map(c => `<option value="${c.id}" ${action.targetCollection === c.id ? 'selected' : ''}>${c.name}</option>`).join(''); row.innerHTML = `<button class="btn-danger btn-delete-action">-</button><div class="form-group"><label>Loại Hành Động</label><select class="action-type-selector"><option value="calculateAssign">Tính Toán & Gán</option><option value="setVariable">Gán Giá Trị cho Biến</option><option value="addToCollection">Thêm Vật Phẩm</option><option value="setItemInSlot">Gán Vào Ô</option><option value="removeItems">Xóa Vật Phẩm</option></select></div><div class="action-fields-container"><div class="calculateAssign-fields"><div class="form-group"><label>Tên biến tạm (VD: _item_count)</label><input type="text" class="action-temp-var-name" value="${action.targetTempVar || ''}"></div><div class="form-row"><div class="form-group"><label>Hàm Tính</label><select class="action-calc-function"><option value="COUNT">Đếm (COUNT)</option></select></div><div class="form-group"><label>Kho Chứa Nguồn</label><select class="action-collection-target">${collectionOptions}</select></div></div></div><div class="setVariable-fields">${createSetVariableActionRow({}).innerHTML.replace('<button class="btn-danger btn-delete-action">-</button>','')}</div><div class="addToCollection-fields"><div class="form-row"><div class="form-group"><label>Kho Chứa</label><select class="action-collection-target">${collectionOptions}</select></div><div class="form-group"><label>Tên vật phẩm</label><input type="text" class="action-collection-value" value="${action.value || ''}" list="item-suggestions"></div></div></div><div class="setItemInSlot-fields"><div class="form-group"><label>Kho Chứa</label><select class="action-collection-target">${collectionOptions}</select></div><div class="form-row"><div class="form-group"><label>Vị trí ô</label><input type="text" class="action-slot-value" placeholder="VD: 1, [CURRENT]+1" value="${action.slot || ''}"></div><div class="form-group"><label>Tên vật phẩm</label><input type="text" class="action-item-value" placeholder="VD: Kiếm Sắt, [NONE]" list="item-suggestions" value="${action.value || ''}"></div></div></div><div class="removeItems-fields"><div class="form-group"><label>Kho Chứa</label><select class="action-collection-target">${collectionOptions}</select></div><div class="form-row"><div class="form-group"><label>Chế độ</label><select class="action-remove-mode"><option value="random">Ngẫu nhiên</option><option value="oldest">Cũ nhất</option><option value="newest">Mới nhất</option></select></div><div class="form-group"><label>Số lượng</label><select class="action-remove-amount-type"><option value="specific">Số lượng cụ thể</option><option value="range">Khoảng ngẫu nhiên</option><option value="all">Toàn bộ</option></select></div></div><div class="remove-items-amount-container"><div class="form-group remove-items-amount-specific"><label>Giá trị</label><input type="text" class="action-remove-amount-v1" placeholder="VD: 2, _biến_tạm"></div><div class="form-row remove-items-amount-range"><div class="form-group"><label>Từ</label><input type="text" class="action-remove-amount-v2" placeholder="VD: 1"></div><div class="form-group"><label>Đến</label><input type="text" class="action-remove-amount-v3" placeholder="VD: 3"></div></div></div></div></div>`; const typeSelector = row.querySelector('.action-type-selector'); const fieldsContainer = row.querySelector('.action-fields-container'); const toggleFields = () => { fieldsContainer.querySelectorAll(':scope > div').forEach(el => el.classList.remove('active')); const activeFields = fieldsContainer.querySelector(`.${typeSelector.value}-fields`); if(activeFields) activeFields.classList.add('active'); }; typeSelector.addEventListener('change', toggleFields); const removeAmountTypeSelect = row.querySelector('.removeItems-fields .action-remove-amount-type'); if (removeAmountTypeSelect) { const amountContainer = row.querySelector('.removeItems-fields .remove-items-amount-container'); const specificInput = amountContainer.querySelector('.remove-items-amount-specific'); const rangeInputs = amountContainer.querySelector('.remove-items-amount-range'); const toggleRemoveAmountInputs = () => { specificInput.style.display = 'none'; rangeInputs.style.display = 'none'; const amountType = removeAmountTypeSelect.value; if (amountType === 'specific') specificInput.style.display = 'grid'; else if (amountType === 'range') rangeInputs.style.display = 'grid'; }; removeAmountTypeSelect.addEventListener('change', toggleRemoveAmountInputs); if (action.type === 'removeItems') toggleRemoveAmountInputs(); } if(action.type) { typeSelector.value = action.type; const fields = row.querySelector(`.${action.type}-fields`); if(fields) { if(action.type === 'setVariable'){ fields.querySelector('.action-var-name').value = action.target || ''; fields.querySelector('.action-var-target-prop').value = action.targetProperty || 'bonus'; fields.querySelector('.action-var-operator').value = action.operator || '='; fields.querySelector('.action-var-value').value = action.value || ''; fields.querySelector('.action-var-min').value = action.min || '1'; fields.querySelector('.action-var-max').value = action.max || '10'; fields.querySelector('.action-var-operator').dispatchEvent(new Event('change')); } else if (action.type === 'removeItems') { fields.querySelector('.action-collection-target').value = action.targetCollection || ''; fields.querySelector('.action-remove-mode').value = action.mode || 'random'; fields.querySelector('.action-remove-amount-type').value = action.amountType || 'specific'; fields.querySelector('.action-remove-amount-v1').value = action.amountValue1 || ''; fields.querySelector('.action-remove-amount-v2').value = action.amountValue2 || ''; fields.querySelector('.action-remove-amount-v3').value = action.amountValue3 || ''; removeAmountTypeSelect.dispatchEvent(new Event('change')); } } } toggleFields(); return row; }
    ...
    function generateWheelSegments(sourceType, filters = {}) { const currentWheel = wheelsData[currentWheelName]; if (!currentWheel) return; let sourceData; if (sourceType === 'itemDatabase') { sourceData = Object.values(itemDatabase); if (filters.category) sourceData = sourceData.filter(item => item.collectionId === filters.category); if (filters.tags && filters.tags.length > 0) sourceData = sourceData.filter(item => filters.tags.every(tag => item.tags && item.tags.includes(tag))); } else if (sourceType === 'entities') { sourceData = Object.values(entities); } else return; if (sourceData.length === 0) { showNotification("Không tìm thấy dữ liệu nào khớp với lựa chọn của bạn.", true); return; } currentWheel.segments = sourceData.map(item => ({ text: item.name, description: item.description || '', weight: 1, color: `#${Math.floor(Math.random()*16777215).toString(16).padStart(6, '0')}`, actions: [] })); showNotification(`Đã tạo thành công ${currentWheel.segments.length} ô mới!`); exitEditMode(); throttledSaveState(); }
    ...
    function endSpinSequenceCleanup() { wheelDisplayArea.classList.remove('is-spinning'); panelOverlay.style.display = 'none'; variableSnapshot = null; tickAudio.pause(); tickAudio.currentTime = 0; updateViewerActions(); }
    ...
    addItemEffectBtn.addEventListener('click', () => { if (activeEditor.type !== 'item') return; const firstVar = Object.keys(variableTemplate)[0]; if(!firstVar) { showNotification("Vui lòng tạo Biến Mẫu trước khi thêm hiệu ứng.", true); return; } itemEffectsContainer.appendChild(createItemEffectRow({target: firstVar, operator: '+=', value: 1})); saveActiveEditor(); });
    ...
    // --- INITIALIZE APP ---
    function initialize() { const savedData = localStorage.getItem('wheelEngineSaveData'); if (savedData) { try { const parsedData = JSON.parse(savedData); showConfirmationModal( "Phát hiện phiên làm việc chưa lưu. Bạn có muốn tải lại không?", () => { loadState(parsedData); }, () => { createDefaultData(); loadWheel(Object.keys(wheelsData)[0]); }); } catch(e) { createDefaultData(); loadWheel(Object.keys(wheelsData)[0]); } } else { createDefaultData(); loadWheel(Object.keys(wheelsData)[0]); } }
    function createDefaultData() { projectSettings = {}; variableTemplate = { suc_manh: { base: 10, bonus: 0, icon: '', displayInStats: true }, nhanh_nhen: { base: 8, bonus: 0, icon: '', displayInStats: true }, mau_hien_tai: { base: 100, bonus: 0, icon: '', displayInStats: true }, luot_quay: { base: 1, bonus: 0, icon: '', displayInStats: false } }; collectionTemplate = { 'Trang_Bi': {id: 'Trang_Bi', name: 'Trang Bị'}, 'Ky_Nang': {id: 'Ky_Nang', name: 'Kỹ Năng'} }; tagDatabase = { 'tag_1': { id: 'tag_1', name: 'vũ khí', icon: ''}, 'tag_2': { id: 'tag_2', name: 'cận chiến', icon: ''}, 'tag_3': { id: 'tag_3', name: 'giáp', icon: ''}, 'tag_4': { id: 'tag_4', name: 'thân', icon: ''} }; itemDatabase = { 'item_1': { id: 'item_1', name: 'Kiếm Sắt', description: 'Một thanh kiếm cơ bản, hơi cùn.\n\n+5 Sức Mạnh', icon: '', effects: [{ target: 'suc_manh', operator: '+=', value: 5 }], tags: ['vũ khí', 'cận chiến'], collectionId: 'Trang_Bi' }, 'item_2': { id: 'item_2', name: 'Giáp Da', description: 'Tăng một chút khả năng phòng thủ.\n\n+20 Máu', icon: '', effects: [{ target: 'mau_hien_tai', operator: '+=', value: 20 }], tags: ['giáp', 'thân'], collectionId: 'Trang_Bi' } }; macros = { 'hut_power': {id: 'hut_power', name: 'Hút Power', actions: [{type: 'calculateAssign', targetTempVar: '_power_count', calcFunction: 'COUNT', sourceCollection: 'Ky_Nang'}, {type: 'setVariable', target: 'suc_manh', targetProperty: 'bonus', operator: '+=', value: 'floor(_power_count / 2)'}, {type: 'removeItems', targetCollection: 'Ky_Nang', amountType: 'all'}]} }; const playerId = `entity_${Date.now()}`; entities = { [playerId]: { id: playerId, name: 'Player', variables: JSON.parse(JSON.stringify(variableTemplate)), avatar: '', collections: {'Trang_Bi': ['Kiếm Sắt', null], 'Ky_Nang': ['Skill 1', 'Skill 2', 'Skill 3']} } }; activeEntityId = playerId; wheelsData['Vong_Quay_Chinh'] = { settings: { spinCountVariable: 'luot_quay' }, segments: [ { text: 'Tấn Công', description: 'Gây sát thương lên mục tiêu.', weight: 1, color: '#f94144', actions: [{ type: 'setVariable', target: 'mau_hien_tai', targetProperty: 'base', operator: '-=', value: '10' }] }, { text: 'Hút Power', description: 'Hy sinh toàn bộ Kỹ Năng để nhận Sức Mạnh.', weight: 1, color: '#43aa8b', actions: [{ type: 'executeMacro', targetMacro: 'hut_power'}] } ] }; computedVariables = []; conditionalRules = [{id: `rule_default`, name: 'Luật Máu Thấp', blocks: [ { type: 'if', conditions: [{type: 'variableCompare', left: 'mau_hien_tai', operator: '<=', right: '30'}], actions: [{type: 'setVariable', target: 'suc_manh', targetProperty: 'bonus', operator: '+=', value: '5'}] } ]}]; updateAllUI(); recalculateActiveEntityStats(); addLogMessage("Chào mừng đến với Wheel Engine v18.1!"); }
